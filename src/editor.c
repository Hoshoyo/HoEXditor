...
internal void render_selection(int num_lines, int num_bytes, int line_written, Font_RenderOutInfo* out_info) {
	float min_y = focused_editor_state->container.maxy - ((font_rendering->max_height) * (float)(num_lines - 1 + 1)) + font_rendering->descent;
	float max_y = focused_editor_state->container.maxy - ((font_rendering->max_height) * (float)(num_lines - 2 + 1)) + font_rendering->descent;
	float max_x = 0.0f;
	float min_x = 0.0f;
	...
	if (focused_editor_state->render) {
		...
		while (num_bytes < _tm_valid_bytes[focused_editor_state->main_buffer_id]) {
			...
			if (num_bytes <= focused_editor_state->cursor_info.selection_offset &&
				...
				in_info.selection_relative_offset = focused_editor_state->cursor_info.selection_offset - num_bytes;
			...
			if (out_info.exited_on_limit_width) {
				...
				if (cursor_line == num_lines) {
					...
					if(in_info.cursor_relative_offset >= 0) focused_editor_state->cursor_info.cursor_column = in_info.cursor_relative_offset;
				}
				...
				queue_text(focused_editor_state->container.minx + offset_x, focused_editor_state->container.maxy - font_rendering->max_height + offset_y, hexbuffer, num_len, 0);
...
internal void render_editor_ascii_mode(Editor_State* es) {
	...
	if (es->render) {
		while (bytes_rendered < es->buffer_valid_bytes - es->cursor_info.block_offset) {
			...
			if (es->selecting) render_selection(num_lines, bytes_rendered, bytes_written, &out_info);
...
internal void handle_key_down_ascii(s32 key, bool selection_reset) {
	...
	if (key == VK_UP) {
		...
		if (CURSOR_RELATIVE_OFFSET - focused_editor_state->cursor_info.cursor_column - 1 < 0) {
			assert(focused_editor_state->cursor_info.cursor_line == 0);
