...
u8* read_entire_file(wchar_t* filename, s64* out_size)
{
	...
	void* memory = memory_alloc(file_size);
...
void error_fatal(char* error_type, char* buffer)
{
	...
	WriteConsoleA(error_handle, error_type, hstrlen(error_type), &written, 0);
	if (buffer) {
		WriteConsoleA(error_handle, buffer, hstrlen(buffer), &written, 0);
	}
