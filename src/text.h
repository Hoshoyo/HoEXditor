...
#define MAX_CONTIGUOUS_TEXT_OPEN 128
...
typedef struct text_id_struct text_id;
...
// aux functions
internal s32 get_empty_id(bool is_block_text);
internal ho_block* put_new_block_and_move_others_to_right(text_id tid, ho_block new_block, ho_block existing_block);
internal ho_block* delete_block_and_move_others_to_left(text_id tid, ho_block block_to_be_deleted);
internal void* fill_arena_bitmap_and_return_address(ho_arena_descriptor* arena_descriptor);
internal ho_block_data request_new_block_data(text_id tid);
internal void free_block_data(text_id tid, ho_block_data block_data);
internal ho_arena_descriptor* create_new_arena(text_id tid, ho_arena_descriptor* last_arena);
internal bool is_arena_empty(ho_arena_descriptor* arena);
internal void free_arena(text_id tid, ho_arena_descriptor* arena);
